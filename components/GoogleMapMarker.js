...
const MapMarker = React.createClass({
  viewConfig: {
    uiViewClassName: 'AIRGoogleMapMarker',
    validAttributes: {
      coordinate: true,
    },
  },
    ...
    switch (Platform.OS) {
      case 'android':
        NativeModules.UIManager.dispatchViewManagerCommand(
          this._getHandle(),
          NativeModules.UIManager.AIRGoogleMapMarker.Commands[name],
          args
        );
        break;

      case 'ios':
        NativeModules.AIRGoogleMapMarkerManager[name].apply(
          NativeModules.AIRGoogleMapMarkerManager[name],
...
const AIRGoogleMapMarker = requireNativeComponent('AIRGoogleMapMarker', MapMarker);
